...
dat$weekly_mortality <-
  ...
  mutate(
    iso_year_has_week_53 = YearHasIsoWeek53(iso_year),
    ...
    epi_week =
      case_when(
        iso_year_has_week_53 ~ ISOWeekToEpiWeek(
          iso_week,
          w_start = cnst$config$forecast$start_of_epi_year_iso_week,
...
dat$cv_selection <-
  tibble(
    ...
    training_start =
      ISOWeekDateToDate(
        2001+c(0, 4, 8),
        week = cnst$config$forecast$start_of_training_iso_week
      ),
    test_start =
      ISOWeekDateToDate(
        2008+c(0, 4, 8),
...
dat$total_selection <-
  tibble(
    ...
    training_start =
      ISOWeekDateToDate(
        2013, week = cnst$config$forecast$start_of_training_iso_week
      ),
    test_start =
      ISOWeekDateToDate(
...
ggsave(
  filename = 'training_test_split.pdf',
  path = 'tmp',
