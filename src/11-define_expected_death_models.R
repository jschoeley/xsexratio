...
ModDef$CountGAM <- function (
  ...
  for (i in strata) {
    ...
    # add predictions and simulations to input data
    df[df_prd[['.rowid']], 'deaths_predicted'] <- Ey_prd
    df[df_prd[['.rowid']], colnames_y_sim] <- y_sim
...
saveRDS(ModDef, file = path$mod_def)
